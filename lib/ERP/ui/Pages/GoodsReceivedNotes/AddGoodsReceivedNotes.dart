...
import '../../../bloc/GoodsReceivedNotesBloc/goods_received_notes_by_id_bloc.dart';
import '../../../bloc/GoodsReceivedNotesBloc/grn_detail_by_id_bloc.dart';
...
class _AddGoodsReceivedNotesState extends State<AddGoodsReceivedNotes> {
  ...
  void _onSavePressed() {
    ...
    final itemNames =
    grnItems.map((e) => e.name).join(',');
    final quantities =
    grnItems.map((e) => e.orderedQty.toString()).join(',');
    final receivedQty =
    grnItems.map((e) => e.receivedQty.text).join(',');
    final shortQty =
    grnItems.map((e) => e.shortQty.text).join(',');
    final excessQty =
    grnItems.map((e) => e.excessQty.text).join(',');
    final rejectedQty = grnItems
        .map((e) => e.rejectedQty.text.isNotEmpty ? e.rejectedQty.text : '0')
        .join(',');
